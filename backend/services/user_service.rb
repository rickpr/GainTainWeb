...
class UserService < Gaintain::UserService::Service
  def list_users(_unused_request, _unused_call)
    Database.with_database { Gaintain::Users.new(users: User.all.map(&:to_proto)) }
  end

  def get_user(user_request, _unused_call)
    Database.with_database { User.find(user_request.id).to_proto }
  end
  ...
  def delete_user(user_request, _unused_call)
    Datbase.with_database { User.find(user_request.id).destroy! }
  end
end
